...
<?php include_once "hoa_donrptinfo.php" ?>
<?php
...
$hoa_don_rpt = NULL; // Initialize page object first

class crhoa_don_rpt extends crhoa_don {
	...
	function __construct() {
		...
		// Table object (hoa_don)
		if (!isset($GLOBALS["hoa_don"])) {
			$GLOBALS["hoa_don"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["hoa_don"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'hoa_don', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fhoa_donrpt";
...
//		$item->Visible = FALSE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_hoa_don\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_hoa_don',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fhoa_donrpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fhoa_donrpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fhoa_donrpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = FALSE;
	...
	function GetRow($opt) {
		...
		if (!$rs->EOF) {
			...
			$this->Val[2] = $this->TenTT->CurrentValue;
			$this->Val[3] = $this->TenVC->CurrentValue;
			$this->Val[4] = $this->NgayLap->CurrentValue;
			$this->Val[5] = $this->TongTien->CurrentValue;
			$this->Val[6] = $this->TinhTrang->CurrentValue;
		} else {
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			$this->MaHD->HrefValue = "";
			if ($this->Export == "") {
				$drillurl = $this->MaHD->DrillDownUrl;
				$drillurl = str_replace("=f0", "=" . ewr_Encrypt($this->GetDrillDownSQL($this->MaHD, "MaHD", $this->RowTotalType, -1)), $drillurl);
				$this->MaHD->LinkAttrs["title"] = ewr_JsEncode($GLOBALS["ReportLanguage"]->Phrase("ClickToDrillDown"));
				$this->MaHD->LinkAttrs["class"] = "ewDrillLink";
				$this->MaHD->LinkAttrs["onclick"] = ewr_DrillDownJs($drillurl, 'hoa_don_MaHD', $GLOBALS["ReportLanguage"]->TablePhrase('chi_tiet_hoa_don', 'TblCaption'), $this->UseDrillDownPanel);
			}
			...
			if ($this->Export == "") {
				$drillurl = $this->MaHD->DrillDownUrl;
				$drillurl = str_replace("=f0", "=" . ewr_Encrypt($this->GetDrillDownSQL($this->MaHD, "MaHD", 0)), $drillurl);
				$this->MaHD->LinkAttrs["title"] = ewr_JsEncode($ReportLanguage->Phrase("ClickToDrillDown"));
				$this->MaHD->LinkAttrs["class"] = "ewDrillLink";
				$this->MaHD->LinkAttrs["onclick"] = ewr_DrillDownJs($drillurl, 'hoa_don_MaHD', $GLOBALS["ReportLanguage"]->TablePhrase('chi_tiet_hoa_don', 'TblCaption'), $this->UseDrillDownPanel);
			}
	...
	function GetDrillDownFilter() {
		...
		if ($opt == "1" || $opt == "2") {
			...
			$_SESSION['do_hoa_don'] = $opt;
			$_SESSION['df_hoa_don'] = $filter;
			$_SESSION['dl_hoa_don'] = $sFilterList;
			...
			$opt = @$_SESSION['do_hoa_don'];
			$filter = @$_SESSION['df_hoa_don'];
			$sFilterList = @$_SESSION['dl_hoa_don'];
		}
		if ($opt == "1" || $opt == "2")
			$this->DrillDown = TRUE;
		if ($opt == "1") {
...
// Create page object
if (!isset($hoa_don_rpt)) $hoa_don_rpt = new crhoa_don_rpt();
...
// Create page object
var hoa_don_rpt = new ewr_Page("hoa_don_rpt");

// Page properties
hoa_don_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = hoa_don_rpt.PageID;

// Extend page with Chart_Rendering function
hoa_don_rpt.Chart_Rendering = 
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="MaHD"><div class="hoa_don_MaHD"><span class="ewTableHeaderCaption"><?php echo $Page->MaHD->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="MaHD">
<?php if ($Page->SortUrl($Page->MaHD) == "") { ?>
		<div class="ewTableHeaderBtn hoa_don_MaHD">
			<span class="ewTableHeaderCaption"><?php echo $Page->MaHD->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer hoa_don_MaHD" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->MaHD) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->MaHD->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="TenTT"><div class="hoa_don_TenTT"><span class="ewTableHeaderCaption"><?php echo $Page->TenTT->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="TenTT">
<?php if ($Page->SortUrl($Page->TenTT) == "") { ?>
		<div class="ewTableHeaderBtn hoa_don_TenTT">
			<span class="ewTableHeaderCaption"><?php echo $Page->TenTT->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer hoa_don_TenTT" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->TenTT) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->TenTT->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="TenVC"><div class="hoa_don_TenVC"><span class="ewTableHeaderCaption"><?php echo $Page->TenVC->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="TenVC">
<?php if ($Page->SortUrl($Page->TenVC) == "") { ?>
		<div class="ewTableHeaderBtn hoa_don_TenVC">
			<span class="ewTableHeaderCaption"><?php echo $Page->TenVC->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer hoa_don_TenVC" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->TenVC) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->TenVC->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="NgayLap"><div class="hoa_don_NgayLap"><span class="ewTableHeaderCaption"><?php echo $Page->NgayLap->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="NgayLap">
<?php if ($Page->SortUrl($Page->NgayLap) == "") { ?>
		<div class="ewTableHeaderBtn hoa_don_NgayLap">
			<span class="ewTableHeaderCaption"><?php echo $Page->NgayLap->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer hoa_don_NgayLap" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->NgayLap) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->NgayLap->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="TongTien"><div class="hoa_don_TongTien"><span class="ewTableHeaderCaption"><?php echo $Page->TongTien->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="TongTien">
<?php if ($Page->SortUrl($Page->TongTien) == "") { ?>
		<div class="ewTableHeaderBtn hoa_don_TongTien">
			<span class="ewTableHeaderCaption"><?php echo $Page->TongTien->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer hoa_don_TongTien" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->TongTien) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->TongTien->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="TinhTrang"><div class="hoa_don_TinhTrang"><span class="ewTableHeaderCaption"><?php echo $Page->TinhTrang->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="TinhTrang">
<?php if ($Page->SortUrl($Page->TinhTrang) == "") { ?>
		<div class="ewTableHeaderBtn hoa_don_TinhTrang">
			<span class="ewTableHeaderCaption"><?php echo $Page->TinhTrang->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer hoa_don_TinhTrang" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->TinhTrang) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->TinhTrang->FldCaption() ?></span>
...
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_hoa_don_MaHD"<?php echo $Page->MaHD->ViewAttributes() ?>>
<?php if ($Page->MaHD->HrefValue <> "" || @$Page->MaHD->LinkAttrs["onclick"] <> "") { ?>
<?php if ($Page->MaHD->ListViewValue() <> "" && $Page->MaHD->ListViewValue() <> "&nbsp;") { ?>
<a<?php echo $Page->MaHD->LinkAttributes() ?>><?php echo $Page->MaHD->ListViewValue() ?></a>
<?php } else { echo "&nbsp;"; } ?>
<?php } else { ?>
<?php if ($Page->MaHD->ListViewValue() <> "" && $Page->MaHD->ListViewValue() <> "&nbsp;") { ?>
<?php echo $Page->MaHD->ListViewValue() ?>
<?php } else { echo "&nbsp;"; } ?>
<?php } ?>
</span></td>
<?php } ?>
...
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_hoa_don_TenTT"<?php echo $Page->TenTT->ViewAttributes() ?>><?php echo $Page->TenTT->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->TenVC->Visible) { ?>
		<td data-field="TenVC"<?php echo $Page->TenVC->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_hoa_don_TenVC"<?php echo $Page->TenVC->ViewAttributes() ?>><?php echo $Page->TenVC->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->NgayLap->Visible) { ?>
		<td data-field="NgayLap"<?php echo $Page->NgayLap->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_hoa_don_NgayLap"<?php echo $Page->NgayLap->ViewAttributes() ?>><?php echo $Page->NgayLap->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->TongTien->Visible) { ?>
		<td data-field="TongTien"<?php echo $Page->TongTien->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_hoa_don_TongTien"<?php echo $Page->TongTien->ViewAttributes() ?>><?php echo $Page->TongTien->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->TinhTrang->Visible) { ?>
		<td data-field="TinhTrang"<?php echo $Page->TinhTrang->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_hoa_don_TinhTrang"<?php echo $Page->TinhTrang->ViewAttributes() ?>><?php echo $Page->TinhTrang->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
...
<?php include "hoa_donrptpager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
</div>
