...
<?php include_once "chi_tiet_hoa_donrptinfo.php" ?>
<?php
...
$chi_tiet_hoa_don_rpt = NULL; // Initialize page object first

class crchi_tiet_hoa_don_rpt extends crchi_tiet_hoa_don {
	...
	// Page object name
	var $PageObjName = 'chi_tiet_hoa_don_rpt';
	...
	function __construct() {
		...
		// Table object (chi_tiet_hoa_don)
		if (!isset($GLOBALS["chi_tiet_hoa_don"])) {
			$GLOBALS["chi_tiet_hoa_don"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["chi_tiet_hoa_don"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'chi_tiet_hoa_don', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fchi_tiet_hoa_donrpt";
...
//		$item->Visible = FALSE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_chi_tiet_hoa_don\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_chi_tiet_hoa_don',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fchi_tiet_hoa_donrpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fchi_tiet_hoa_donrpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fchi_tiet_hoa_donrpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = FALSE;
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
			$rs->MoveFirst(); // Move first
				...
				$this->FirstRowData['ThanhTien'] = ewr_Conv($rs->fields('ThanhTien'), 3);
		...
		if (!$rs->EOF) {
			...
			$this->ThanhTien->setDbValue($rs->fields('ThanhTien'));
			$this->MaHD->setDbValue($rs->fields('MaHD'));
			$this->Val[1] = $this->TenSP->CurrentValue;
			$this->Val[2] = $this->TenThuongHieu->CurrentValue;
			$this->Val[3] = $this->TenLoaiSP->CurrentValue;
			$this->Val[4] = $this->SoLuong->CurrentValue;
			$this->Val[5] = $this->ThanhTien->CurrentValue;
		} else {
			$this->TenSP->setDbValue("");
			$this->TenThuongHieu->setDbValue("");
			$this->TenLoaiSP->setDbValue("");
			$this->SoLuong->setDbValue("");
			$this->ThanhTien->setDbValue("");
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			// ThanhTien
			$this->ThanhTien->ViewValue = $this->ThanhTien->CurrentValue;
			$this->ThanhTien->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// ThanhTien
			$CurrentValue = $this->ThanhTien->CurrentValue;
			$ViewValue = &$this->ThanhTien->ViewValue;
			$ViewAttrs = &$this->ThanhTien->ViewAttrs;
			$CellAttrs = &$this->ThanhTien->CellAttrs;
			$HrefValue = &$this->ThanhTien->HrefValue;
			$LinkAttrs = &$this->ThanhTien->LinkAttrs;
			$this->Cell_Rendered($this->ThanhTien, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
		}
	...
	function SetupFieldCount() {
		...
		if ($this->ThanhTien->Visible) $this->DtlColumnCount += 1;
	}
	...
	function GetDrillDownFilter() {
		...
		if ($opt == "1" || $opt == "2") {
			...
			$sql = @$post["MaHD"];
			$sql = ewr_Decrypt($sql);
			$sql = str_replace("@MaHD", "`MaHD`", $sql);
			if ($sql <> "") {
				if ($filter <> "") $filter .= " AND ";
				$filter .= $sql;
				if ($sql <> "1=1")
					$sFilterList .= "<div><span class=\"ewFilterCaption\">" . $this->MaHD->FldCaption() . "</span><span class=\"ewFilterValue\">$sql</span></div>";
			}

			// Save to session
			$_SESSION[EWR_PROJECT_NAME . "_" . $this->TableVar . "_" . EWR_TABLE_MASTER_TABLE] = $mastertable;
			$_SESSION['do_chi_tiet_hoa_don'] = $opt;
			$_SESSION['df_chi_tiet_hoa_don'] = $filter;
			$_SESSION['dl_chi_tiet_hoa_don'] = $sFilterList;
		} elseif (@$_GET["cmd"] == "resetdrilldown") { // Clear drill down
			$_SESSION[EWR_PROJECT_NAME . "_" . $this->TableVar . "_" . EWR_TABLE_MASTER_TABLE] = "";
			$_SESSION['do_chi_tiet_hoa_don'] = "";
			$_SESSION['df_chi_tiet_hoa_don'] = "";
			$_SESSION['dl_chi_tiet_hoa_don'] = "";
		} else { // Restore from Session
			$opt = @$_SESSION['do_chi_tiet_hoa_don'];
			$filter = @$_SESSION['df_chi_tiet_hoa_don'];
			$sFilterList = @$_SESSION['dl_chi_tiet_hoa_don'];
		}
		if ($opt == "1" || $opt == "2")
			$this->DrillDown = TRUE;
		if ($opt == "1") {
...
// Create page object
if (!isset($chi_tiet_hoa_don_rpt)) $chi_tiet_hoa_don_rpt = new crchi_tiet_hoa_don_rpt();
if (isset($Page)) $OldPage = $Page;
$Page = &$chi_tiet_hoa_don_rpt;
...
// Create page object
var chi_tiet_hoa_don_rpt = new ewr_Page("chi_tiet_hoa_don_rpt");

// Page properties
chi_tiet_hoa_don_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = chi_tiet_hoa_don_rpt.PageID;

// Extend page with Chart_Rendering function
chi_tiet_hoa_don_rpt.Chart_Rendering = 
...
chi_tiet_hoa_don_rpt.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="TenSP"><div class="chi_tiet_hoa_don_TenSP"><span class="ewTableHeaderCaption"><?php echo $Page->TenSP->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="TenSP">
<?php if ($Page->SortUrl($Page->TenSP) == "") { ?>
		<div class="ewTableHeaderBtn chi_tiet_hoa_don_TenSP">
			<span class="ewTableHeaderCaption"><?php echo $Page->TenSP->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer chi_tiet_hoa_don_TenSP" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->TenSP) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->TenSP->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="TenThuongHieu"><div class="chi_tiet_hoa_don_TenThuongHieu"><span class="ewTableHeaderCaption"><?php echo $Page->TenThuongHieu->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="TenThuongHieu">
<?php if ($Page->SortUrl($Page->TenThuongHieu) == "") { ?>
		<div class="ewTableHeaderBtn chi_tiet_hoa_don_TenThuongHieu">
			<span class="ewTableHeaderCaption"><?php echo $Page->TenThuongHieu->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer chi_tiet_hoa_don_TenThuongHieu" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->TenThuongHieu) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->TenThuongHieu->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="TenLoaiSP"><div class="chi_tiet_hoa_don_TenLoaiSP"><span class="ewTableHeaderCaption"><?php echo $Page->TenLoaiSP->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="TenLoaiSP">
<?php if ($Page->SortUrl($Page->TenLoaiSP) == "") { ?>
		<div class="ewTableHeaderBtn chi_tiet_hoa_don_TenLoaiSP">
			<span class="ewTableHeaderCaption"><?php echo $Page->TenLoaiSP->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer chi_tiet_hoa_don_TenLoaiSP" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->TenLoaiSP) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->TenLoaiSP->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="SoLuong"><div class="chi_tiet_hoa_don_SoLuong"><span class="ewTableHeaderCaption"><?php echo $Page->SoLuong->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="SoLuong">
<?php if ($Page->SortUrl($Page->SoLuong) == "") { ?>
		<div class="ewTableHeaderBtn chi_tiet_hoa_don_SoLuong">
			<span class="ewTableHeaderCaption"><?php echo $Page->SoLuong->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer chi_tiet_hoa_don_SoLuong" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->SoLuong) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->SoLuong->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->ThanhTien->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="ThanhTien"><div class="chi_tiet_hoa_don_ThanhTien"><span class="ewTableHeaderCaption"><?php echo $Page->ThanhTien->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="ThanhTien">
<?php if ($Page->SortUrl($Page->ThanhTien) == "") { ?>
		<div class="ewTableHeaderBtn chi_tiet_hoa_don_ThanhTien">
			<span class="ewTableHeaderCaption"><?php echo $Page->ThanhTien->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer chi_tiet_hoa_don_ThanhTien" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->ThanhTien) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->ThanhTien->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->ThanhTien->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->ThanhTien->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_chi_tiet_hoa_don_TenSP"<?php echo $Page->TenSP->ViewAttributes() ?>><?php echo $Page->TenSP->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->TenThuongHieu->Visible) { ?>
		<td data-field="TenThuongHieu"<?php echo $Page->TenThuongHieu->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_chi_tiet_hoa_don_TenThuongHieu"<?php echo $Page->TenThuongHieu->ViewAttributes() ?>><?php echo $Page->TenThuongHieu->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->TenLoaiSP->Visible) { ?>
		<td data-field="TenLoaiSP"<?php echo $Page->TenLoaiSP->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_chi_tiet_hoa_don_TenLoaiSP"<?php echo $Page->TenLoaiSP->ViewAttributes() ?>><?php echo $Page->TenLoaiSP->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->SoLuong->Visible) { ?>
		<td data-field="SoLuong"<?php echo $Page->SoLuong->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_chi_tiet_hoa_don_SoLuong"<?php echo $Page->SoLuong->ViewAttributes() ?>><?php echo $Page->SoLuong->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->ThanhTien->Visible) { ?>
		<td data-field="ThanhTien"<?php echo $Page->ThanhTien->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_chi_tiet_hoa_don_ThanhTien"<?php echo $Page->ThanhTien->ViewAttributes() ?>><?php echo $Page->ThanhTien->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
...
<?php include "chi_tiet_hoa_donrptpager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
</div>
